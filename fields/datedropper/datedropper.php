...
function fl_datedropper_field($name, $value, $field, $settings) {
    ...
    echo '<script src="'. FL_MODULE_THEME_URL . 'fields/datedropper/datedropper.js'.'"></script>';
}
add_action( 'wp_enqueue_scripts', 'fl_datedropper' );
function fl_datedropper() {
    if ( class_exists( 'FLBuilderModel' ) && FLBuilderModel::is_builder_active() ) {
        wp_enqueue_style( 'fl-datedropper-fields', FL_MODULE_THEME_URL . 'fields/datedropper/datedropper.css', array(), '' );
    }
}
?>