...
function fl_timedropper_field($name, $value, $field, $settings) {
    ...
    echo '<script src="'. FL_MODULE_THEME_URL . 'fields/timedropper/timedropper.js'.'"></script>';
}
add_action( 'wp_enqueue_scripts', 'fl_timedropper' );
function fl_timedropper() {
    if ( class_exists( 'FLBuilderModel' ) && FLBuilderModel::is_builder_active() ) {
        wp_enqueue_style( 'fl-timedropper-fields', FL_MODULE_THEME_URL . 'fields/timedropper/timedropper.css', array(), '' );
    }
}
?>