...
$output = '<div class="'.$module->get_classname().'">';
	if ( $settings->source != 'media-gallery' ) {
		if ( $settings->categories_nav != 'hide' ) {
			...
			$output .= '<ul class="portfolio-filter'.$button_aligment.'">';
				$button_all = $settings->categories_nav_all_text <> '' ? $settings->categories_nav_all_text : 'All';
				$output .= '<li class="portfolio-button filter-'.$id.'" data-filter="all">'. $button_all .'</li>';
				if ( $settings->categories != 'selected' || $settings->selected_categories == '' ) {
					...
					if($count > 0){
						foreach($terms as $term){
							$output .= '<li class="portfolio-button filter-'.$id.'" data-filter=".'.$term->slug.'">'.$term->name.'</li>';
						}
					}
				} else {
					...
					if($count > 0){
						foreach($terms as $term){
							$output .= '<li class="portfolio-button filter-'.$id.'" data-filter=".'.$term.'">'.$term.'</li>';
						}
					}
				}
			$output .= '</ul>';
	...
	$output .= '<div class="grids portfolio-items">';
		if ( $settings->source != 'media-gallery' ) {
			...
			while ($query->have_posts()) : $query->the_post();
				...
				$allClasses = get_the_terms( get_the_ID(), 'portfolio_category' ); 
				if(! empty($allClasses)){
					...
					$output .= '<figure>'; 
						...
						if ( $settings->image_link == 'yes' ) {
							if ( $settings->button_link == 'lightbox-ajax' ) {
								$output .=  '<a class="ajax-content portfolio-mixitup-photo" href="'.$buttonlink.'" title="View '.get_the_title().'" data-post-id="'.$postID.'" data-post-slug="'.$postSLUG.'">';
							} else {
								$output .=  '<a class="portfolio-mixitup-photo" href="'.$buttonlink.'" title="View '.get_the_title().'">';
							}
						} else {
							$output .=  '<div class="portfolio-mixitup-photo">';
						}
							if ( $grid == 'grid-12') {
								...
								$img = wp_get_attachment_image_src( get_post_thumbnail_id( $postID ), 'portfolio-mixitup-large' );
							} else {
								$img = wp_get_attachment_image_src( get_post_thumbnail_id( $postID ), 'portfolio-mixitup' );
							}
							$imgSRC = $img <> '' ? $img[0] : $module->url .'/img/default.svg';
						    $imgSRCClass = $img <> '' ? ' has-image' : ' no-image';
						    $output .= '<div class="portfolio-item-image'.$imgSRCClass.'" style="background-image: url('.$imgSRC.');"><img src="'.$imgSRC.'"></div>'; 
						if ( $settings->image_link == 'yes' ) {
							$output .=  '</a>'; 
						} else {
							$output .=  '</div>'; 
						...
						if ( $settings->grid_style != ' none') {
							...
							$output .=  '<figcaption'.$figcaption_classes.'>'; 
								$output .=  '<'.$settings->grid_info_title_tag.' class="portfolio-title">'.get_the_title().'</'.$settings->grid_info_title_tag.'>'; 
								if ( $settings->grid_category == 'enable' ) {
									$allCat = get_the_terms( get_the_ID(), 'portfolio_category' ); 
								...
								if ( $settings->grid_desc == 'enable' ) {
									if ( $settings->grid_style == ' blurb' || $settings->grid_style == ' highlight' ) {
										$output .=  '<p class="portfolio-desc">'.get_the_excerpt().'</p>';
									}
								}
								if ( $settings->grid_button_text <> '' ) {
									if ( $settings->button_link == 'lightbox-ajax' ) {
										$output .=  '<a href="'.$buttonlink.'" title="'.$settings->grid_button_text.'" class="ajax-content portfolio-button" data-post-id="'.$postID.'" data-post-slug="'.$postSLUG.'">'.$settings->grid_button_text.'</a>'; 
									} else {
										$output .=  '<a href="'.$buttonlink.'" title="'.$settings->grid_button_text.'" class="portfolio-button">'.$settings->grid_button_text.'</a>'; 
									}
								}
							$output .=  '</figcaption>';
						}
			...
			if ( !empty($images) ) {
				foreach ( $images as $i => $image ) {
					...
					if ( $grid == 'grid-12') {
						...
						$photo_thumb = wp_get_attachment_image_src($image, 'portfolio-mixitup-large' );
					} else {
						$photo_thumb = wp_get_attachment_image_src($image, 'portfolio-mixitup' );
					}
					$output .= '<article class="mix '.$grid.'">'; 
						$output .= '<figure>';
							$buttonlink = ''; $lightboximg = wp_get_attachment_image_src($image, 'medium-large' ); $buttonlink = $lightboximg[0];
							if ( $settings->image_link == 'yes' ) {
								$output .=  '<a class="portfolio-mixitup-photo" href="'.$buttonlink.'" title="View '.$photo_title.'">'; 
							} else {
								$output .=  '<div class="portfolio-mixitup-photo">';
							}
							...
							if ( $settings->grid_style != ' none') {
								$output .=  '<figcaption>'; 
									$output .=  '<'.$settings->grid_info_title_tag.' class="portfolio-title">'.$photo_title.'</'.$settings->grid_info_title_tag.'>'; 
									if ( !empty($photo_caption) ) {
										$output .=  '<p class="portfolio-caption">'.$photo_caption.'</p>';
									}
